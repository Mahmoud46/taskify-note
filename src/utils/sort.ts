...
const assignPriority = (item: INote): number => {
	...
	const statusScore = item.status
		? statusPriority[item.status as TTaskStatus]
		: 4; // status priority
	const timeScore =
		item.date && item.time
			? parseDateTime(item.date, item.time)
			: Number.MAX_SAFE_INTEGER; // earlier time first

	// Compose into a weighted priority score
	return (
		favoriteScore * 1e13 + typeScore * 1e12 + statusScore * 1e10 + timeScore
	);
};
